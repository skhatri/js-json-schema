  ...
  if (type.toLowerCase() === "enum") {
    ...
    if (!isDefaultType(schema.properties[name].type.toLowerCase())) {
      ...
      if (refType.endsWith("[]")) {
        ...
        var typeName = refType.slice(0, refType.length - 2);
        if (isDefaultType(typeName.toLowerCase())) {
          schema.properties[name]["items"] = {"type": typeName.toLowerCase()};
        } else {
          schema.properties[name]["items"] = {"$ref": typeName + options.ext};
        }
      } else {
